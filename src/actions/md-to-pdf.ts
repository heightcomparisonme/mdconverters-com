...
import { convertMarkdownToPdf, type MdToPdfConfig } from '@/lib/md-to-pdf';
import { z } from 'zod';
...
export const convertMdToPdfAction = actionClient
	...
	.action(async ({ parsedInput: { markdown, config } }) => {
		try {
			const result = await convertMarkdownToPdf(markdown, config as MdToPdfConfig);
			...
			if (!base64Pdf || typeof base64Pdf !== 'string') {
				console.error('Invalid base64 generated:', typeof base64Pdf, base64Pdf?.length);
				return {
					success: false,
			...
			console.log('PDF converted successfully, base64 length:', base64Pdf.length);
